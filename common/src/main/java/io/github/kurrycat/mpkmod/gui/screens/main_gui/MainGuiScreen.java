...
public class MainGuiScreen extends MPKGuiScreen implements MouseInputListener, MouseScrollListener, KeyInputListener, MessageReceiver {
    public OptionsPane optionsPane = null;
    public LoadConfigPane loadConfigPane = null;
    public SaveConfigPane saveConfigPane = null;

    private final Container guiContainer = new Container();
    private final HudContainer hudContainer = new HudContainer();
    ...
    public void addGuiComponent(Component component) {
        guiContainer.addChild(component);
    }

    @Override
    public void removeGuiComponent(Component component) {
        guiContainer.removeChild(component);
    }

    public void addHudComponent(HudComponent hudComponent) {
        LabelConfiguration.currentConfig.components.add(hudComponent);
        reloadConfig();
    }

    public void removeHudComponent(HudComponent hudComponent) {
        LabelConfiguration.currentConfig.components.remove(hudComponent);
        reloadConfig();
    }

    public List<HudComponent> getHudComponents() {
        return hudContainer.getHudChildren();
    }

    public void reloadConfig() {
        hudContainer.clearChildren();
        LabelConfiguration.currentConfig.components
                .forEach(hudContainer::addChild);
    }
    ...
    public void onGuiInit() {
        ...
        guiContainer.setPos(Vector2D.ZERO);
        guiContainer.setSize(Vector2D.ONE);
        guiContainer.setPercentFlag(PERCENT.SIZE);
        guiContainer.clearChildren();

        hudContainer.setPos(Vector2D.ZERO);
        hudContainer.setSize(Vector2D.ONE);
        hudContainer.setPercentFlag(PERCENT.SIZE);
        hudContainer.clearChildren();
    ...
    public void render(Vector2D mouse, float partialTicks) {
        ...
        getHudComponents().forEach(c -> {
            c.setSelected(selected.contains(c));
            c.setHighlighted(highlighted.contains(c));
        });
        ...
        for (Component c : guiContainer.getChildren()) c.render(hoverMousePos);
    ...
    public void postMessage(String receiverID, String content, boolean highlighted) {
        MessageQueue q = MessageQueue.getReceiverFor(receiverID, ItrUtil.getAllOfType(MessageQueue.class, hudContainer));
        if (q != null)
    ...
    public void onMouseClicked(Vector2D mouse, int mouseButton) {
        ...
        if (hudContainer.getHudChildren().isEmpty()) return;
    ...
    public void onMouseClickMove(Vector2D mouse, int mouseButton, long timeSinceLastClick) {
        ...
        if (getHudComponents().isEmpty()) return;
    ...
    public ArrayList<HudComponent> overlap(Vector2D p1, Vector2D p2) {
        return getHudComponents().stream().filter(
                c -> {
    ...
    public boolean handleMouseScroll(Vector2D mousePos, int delta) {
        ...
        return ItrUtil.orMap(
                ItrUtil.getAllOfType(MouseScrollListener.class, guiContainer.getChildren(), hudContainer.getChildren()),
    ...
    public boolean handleKeyInput(int keyCode, int scanCode, int modifiers, boolean isCharTyped) {
        ...
        return ItrUtil.orMap(
                ItrUtil.getAllOfType(KeyInputListener.class, guiContainer.getChildren(), hudContainer.getChildren()),
    ...
    public boolean handleMouseInput(Mouse.State state, Vector2D mousePos, Mouse.Button button) {
        ...
        return ItrUtil.orMap(
                ItrUtil.getAllOfType(MouseInputListener.class, guiContainer.getChildren(), hudContainer.getChildren()),
    ...
    public ArrayList<HudComponent> findContainPos(Vector2D p) {
        return getHudComponents().stream().filter(c -> c.contains(p)).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public static class HudContainer extends Container {
        public List<HudComponent> getHudChildren() {
            return getChildren().stream()
                    .filter(HudComponent.class::isInstance)
                    .map(HudComponent.class::cast)
        ...
        public void addChild(Component child) {
            throw new UnsupportedOperationException(
                    "HudContainer may only contain HudComponent children; use addChild(HudComponent) instead"
            );
        }
